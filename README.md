...
Type Theory (HOTT) by parametric quantifiers, a first-class notion of Reedy categories and inductive type families
indexed and fibered above them. This combination enables flawless handling of higher categorical objects and
naturally occuring large categories. In particular, HOCC provides simplicial types and other Reedy presheaf types
without reccuring to any kind of non-univalent equality.
...
The term _Construction Calculi_ used in the title of this article refers to type theories that (1) are intended to
...
generated by succesively appending elements to the `Empty` list. Note, that the arrow operator is right-associating,
so `T → (List[T] → List[T])` can be written simply as `T → List[T] → List[T]`.
...
endowed with a (correct by construction, inductively generated) structure of a [Reedy
category](https://ncatlab.org/nlab/show/Reedy+category).
