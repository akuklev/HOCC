...
To make the MLTTs “eat themselves” properly, we need to introduce yet another kind of types not fitting into the landscape set by Voevodski. We start by introducing inductive prototypes, a type-theoretic internalization of Reedy categories, to introduce inductive type families indexed over such prototypes.
...
This paper is a large project seeking for an ultimate proof assistant language. The other paper proposes extending MLTTs with an impredicative universe of propostions (for impredicative reasoning), a virtual reflective universe (for large reasoning) and a natural pair ⁰/□ of modalities that allows parametric reasoning under □-modality and non-constructive reasoning under ⁰-modality. The third paper develops practical syntax for the resulting higher categorical construction calculus. Supplemented by size-guarded recursion and clock-guarded corecursion, it would also yield an ultimate total functional programming language.
...
functions, relations, etc. Such canonical objects are introduced there as synthetic types.
...
Functional types are described not in terms of generators, but in terms of _extractors_. For behavioral types
